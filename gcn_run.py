...
def train(edge_index):
    print('train start...')
    trainLoader.dataset.gcn_ng_sample()
    ...
    for epoch in range(1, P.Epoch+1):
        ...
        for idx, sample in enumerate(tqdm(trainLoader)):
            ...
            pos_score, ng_score = gcnModel(edge_index, head, edge, tail, ng_tail, ng_tail)

            y1 = torch.zeros_like(head, dtype=torch.float32).to(device)

            # 优化器
            optimizer1.zero_grad()
            # 计算预测和样本标签的损失
            # loss1 = gcnLoss(ng_score, pos_score, y1)
            loss1 = gcnLoss(pos_score, y1)
            All_loss += loss1
            # 反向传播
            loss1.backward()
            # 更新参数
...
def evaluate():
    ...
    # test the twoKB part
    for i, golden_triple in enumerate(test_data):
...
if __name__ == '__main__':
    ...
    edge_index, train_data, test_data = LD.Load_gcndata()
    edge_index = edge_index[:2]
    # trainTup = set(trainTup)
    ...
    gcnLoss = nn.MSELoss(reduction='mean')
    # 优化器
